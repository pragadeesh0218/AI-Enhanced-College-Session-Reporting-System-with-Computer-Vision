...
daily_test_time = (current_time + timedelta(minutes=1)).strftime("%H:%M")
schedule.every().day.at(daily_test_time).do(create_and_dispatch_daily_reports)

monthly_test_time = (current_time + timedelta(minutes=1)).strftime("%H:%M")
schedule.every().day.at(monthly_test_time).do(create_and_dispatch_monthly_reports)
